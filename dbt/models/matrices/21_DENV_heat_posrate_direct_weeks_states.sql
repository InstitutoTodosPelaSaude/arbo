...
WITH source_data AS (
    ...
    WHERE
        ...
        test_kit IN ('arbo_pcr_3', 'denv_pcr', 'ns1_antigen') AND
...
states_filtered AS (
    SELECT
        ...
        SUM(CASE WHEN pathogen = 'DENV' THEN "Pos" ELSE 0 END)::int         AS "pos",
        SUM(CASE WHEN pathogen = 'DENV' THEN "Neg" ELSE 0 END)::int         AS "neg"
    FROM source_data
